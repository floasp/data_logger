<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        ...
        if (empty($name) || empty($short_name) || empty($datatypes)) {
            ...
            $sql = "INSERT INTO ID_list (token, name, short_name, datatypes)
                    VALUES ('$token', '$name', '$short_name', '$datatypes')";

            if ($conn->query($sql) === TRUE) {
                // echo "New record created successfully";

                // Get most recently added Sensor and generate a JSON representation.
                $sql = "SELECT t.ID, t.token, t.name, t.short_name, t.datatypes FROM ID_list t
                        INNER JOIN (
